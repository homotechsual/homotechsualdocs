...
if (-not (Test-Path -Path $NinjaGetPath)) {
    New-Item -Path $NinjaGetPath -ItemType Directory -Force | Out-Null
} else {
    Remove-Item -Recurse -Force -Path $NinjaGetPath | Out-Null
    New-Item -Path $NinjaGetPath -ItemType Directory -Force | Out-Null
}
...
Copy-Item -Path ('{0}\ninjaget-dev\*' -f $NinjaGetTempExtractionPath) -Destination $NinjaGetPath -Force -Verbose -Recurse
...
if ((Get-ExecutionPolicy -Scope CurrentUser) -ne 'RemoteSigned') {
    Write-Warning 'Execution policy is not RemoteSigned. Please run the following command to set it permanently, overriding for this process:'
