...
Use `SCREENGRABBER_IMAGE` to choose the rollout channel:

- `homotechsual/screengrabber:latest` for stable releases.
- `homotechsual/screengrabber:edge` for pre-release/main-branch builds.
- `homotechsual/screengrabber:1.0.0` (or `1.0`, `1`) for pinned semver releases.

Equivalent GHCR image names are available under `ghcr.io/homotechsual/screengrabber`.
...
In this repository, in the [`.github/workflows/deploy.yml` file](https://github.com/homotechsual/screengrabber/blob/main/.github/workflows/deploy.yml), you can find the secrets used for deployment and the workflow steps that create this when this project is deployed via SSH using GitHub Actions.
