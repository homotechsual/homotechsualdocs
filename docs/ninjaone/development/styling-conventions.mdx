...
Prefer single quoted strings with the -f operator over string interpolation or concatenation in all cases.
...
# Avoid - interpolation (less clear for complex strings)
$message = "Device $deviceId not found"
...
**Rules**:
- Use `'string {0}' -f $value`.
- Use `-f` for all string formatting for consistency and readability.
- Avoid string concatenation with `+` for clarity.
- Avoid string interpolation for complex strings or when multiple variables are involved.
...
- [ ] All parameters have descriptions using inline comments.
